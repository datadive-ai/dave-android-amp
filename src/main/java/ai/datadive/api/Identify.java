 ...
 *  <br><br>
 *  After creating an Identify object and setting the desired operations, send it to Datadive
 *  servers by calling {@code Datadive.getInstance().identify(identify);} and pass in the object.
 *
 *  @see <a href="https://github.com/datadive/Amplitude-Android#user-properties-and-user-property-operations">
    ...
    public Identify clearAll() {
        if (userPropertiesOperations.length() > 0) {
            if (!userProperties.contains(Constants.AMP_OP_CLEAR_ALL)) {
                DatadiveLog.getLogger().w(TAG, String.format(
        ...
        try {
            ...
            DatadiveLog.getLogger().e(TAG, e.toString());
        }
        return this;
    }
    ...
    private JSONArray floatArrayToJSONArray(float[] values) {
        ...
        for (float value : values) {
            try {
                ...
                DatadiveLog.getLogger().e(TAG, String.format(
