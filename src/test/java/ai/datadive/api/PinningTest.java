...
public class PinningTest extends BaseTest {
    ...
    public void setUp() throws Exception {
        super.setUp(false);
        PinnedDatadiveClient.instances.clear();
    ...
    public void testSslPinning() {
        datadive = PinnedDatadiveClient.getInstance();
        datadive.initialize(context, "1cc2c1978ebab0f6451112a8f5df4f4e");
        ShadowLooper looper = Shadows.shadowOf(datadive.logThread.getLooper());
        looper.runOneTask();
        looper.runOneTask();

        datadive.logEvent("pinned_test_event", null);
        ...
        assertNull(datadive.lastError);
    }

    @Test
    public void testSslPinningInvalid() {
        datadive = new InvalidPinnedDatadiveClient();
        datadive.initialize(context, "1cc2c1978ebab0f6451112a8f5df4f4e");

        ShadowLooper looper = Shadows.shadowOf(datadive.logThread.getLooper());
        looper.runToEndOfTasks();

        datadive.logEvent("pinned_test_event_invalid", null);
        looper.runToEndOfTasks();
        looper.runToEndOfTasks();
        ...
        assertNotNull(datadive.lastError);
    }

    private static class InvalidPinnedDatadiveClient extends PinnedDatadiveClient {
        ...
        public InvalidPinnedDatadiveClient() {
