...
public class BaseTest {
    ...
    protected class DatadiveClientWithTime extends DatadiveClient {
        MockClock mockClock;

        public DatadiveClientWithTime(MockClock mockClock) { this.mockClock = mockClock; }
    ...
    private static final String TEST_PACKAGE_NAME = "ai.datadive.test";
    private static final String TEST_VERSION_NAME = "test_version";

    protected DatadiveClient datadive;
     ...
     * override the defaults by providing an datadive object before
    ...
    public void setUp(boolean withServer) throws Exception {
        ...
        if (datadive == null) {
            datadive = new DatadiveClientWithTime(clock);
        ...
        if (server != null) {
            datadive.url = server.url("/").toString();
        }
    }

    public void tearDown() throws Exception {
        if (datadive != null) {
            datadive.logThread.getLooper().quit();
            datadive.httpThread.getLooper().quit();
            datadive = null;
        }
    ...
    public RecordedRequest runRequest(DatadiveClient datadive) {
        server.enqueue(new MockResponse().setBody("success"));
        ShadowLooper httpLooper = shadowOf(datadive.httpThread.getLooper());
    ...
    public RecordedRequest sendEvent(DatadiveClient datadive, String name, JSONObject props) {
        shadowOf(datadive.logThread.getLooper()).runToEndOfTasks();
        datadive.logEvent(name, props);
        shadowOf(datadive.logThread.getLooper()).runToEndOfTasks();
        shadowOf(datadive.logThread.getLooper()).runToEndOfTasks();

        return runRequest(datadive);
    }

    public RecordedRequest sendIdentify(DatadiveClient datadive, Identify identify) {
        shadowOf(datadive.logThread.getLooper()).runToEndOfTasks();
